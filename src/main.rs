    ...
    .with_event_handler(|state, event, elwt| {
        ...
        match event {
            Event::WindowEvent {
                ...
                camera.render(&universe, buffer.as_mut(), (width, height));

                buffer.present().unwrap();
                window.request_redraw();
                display_time += display_start.elapsed().as_secs_f32();
...
fn handle_key_event(event: KeyEvent, _window: &Window, _camera: &mut Camera) {
    match event {
        KeyEvent {
            logical_key: key,
