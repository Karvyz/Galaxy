...
pub fn to_carthesian(v: &Vec3A) -> Vec3A {
    Vec3A::new(
        v.x * v.y.sin() * v.z.cos(),
        v.x * v.y.sin() * v.z.sin(),
        v.x * v.y.cos(),
...
pub fn to_polar(v: &Vec3A) -> Vec3A {
    Vec3A::new(
        (v.x * v.x + v.y * v.y + v.z * v.z).sqrt(),
        if v.z != 0. {
            if v.z > 0. {
                ((v.x * v.x + v.y * v.y).sqrt() / v.z).atan()
            } else {
                ((v.x * v.x + v.y * v.y).sqrt() / v.z).atan() + PI
            }
        } else if v.x * v.y != 0. {
            PI / 2.
        } else {
...
impl Universe {
    ...
    pub fn add_galaxy(&mut self, position_center: Vec3A, nb_stars: usize, bh_mass_ratio: f32) {
        let black_hole = Star::new(
            position_center,
            Vec3A::ZERO,
            nb_stars as f32 * bh_mass_ratio,
        );
        self.nb_stars += nb_stars;
        for _i in 0..self.nb_stars {
            let pos_star = Vec3A::new(
                gen_gaussian() * 100.,
                gen_gaussian() * 100.,
                gen_gaussian() * 20.,
            );
            ...
            self.stars
                .push(Star::new(pos_star + position_center, Vec3A::ZERO, 1.))
        }
